...
from packet import MCPPacket, ErrorDetails, ValidationResults
...
def test_error_details_structure():
    ...
    print(f"   ✅ Error created successfully")
...
def test_processing_log():
    ...
    print(f"   ✅ Processing log created")
    ...
    if 'processing_log' in packet_dict:
        print(f"   🔄 Log serialization: PASSED")
    else:
        print(f"   ❌ Log serialization: FAILED")
...
async def test_server_integration():
    ...
    try:
        ...
        print(f"   ✅ Server initialized with tripwire validation")
        ...
        if result["success"]:
            print(f"   ✅ Valid packet executed successfully")
        ...
        if not result["success"]:
            print(f"   ✅ Invalid service correctly caught")
            print(f"   🚨 Error: {result.get('error', 'Unknown error')}")
            
            # Check if packet details are returned
            if "packet" in result:
                packet_data = result["packet"]
                print(f"   📦 Packet returned with error details")
                ...
                print(f"   ⚠️  No packet details returned")
        else:
        ...
        if not result["success"]:
            print(f"   ✅ Invalid action correctly caught")
            print(f"   🚨 Error: {result.get('error', 'Unknown error')}")
        else:
            print(f"   ❌ Invalid action should have failed")
            return False
        
        print(f"   ✅ Server integration tests passed")
        return True
